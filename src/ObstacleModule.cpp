...
template<typename T>
bool ObstacleModule::checkObstacle(T *m_sensorManager){

	for (auto i = 0; i < m_pixelsToCheckCount; i++){
		auto currentX = createRandomXValue();
		auto currentY = createRandomYValue();
		auto currentPixelDepth = checkPixelDepth(currentX, currentY, m_sensorManager);
...
template<typename T>
double ObstacleModule::checkPixelDepth(int x, int y, T *m_sensorManager){
